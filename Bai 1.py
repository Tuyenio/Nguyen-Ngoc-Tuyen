...
cur.execute('''
    CREATE TABLE IF NOT EXISTS users (
        ...
        returned_books INTEGER DEFAULT 0, 
        is_active INTEGER DEFAULT 1
    );
''')

# Tạo bảng borrow_history
cur.execute('''
...
class LibraryApp:
    ...
    def manage_users(self):
        ...
        # Hàm xem thông tin lịch sử mượn sách của sinh viên
        def view_borrow_history():
            selected = users_tree.focus()
            values = users_tree.item(selected, 'values')
            if not values:
                messagebox.showinfo("Thông Báo", "Vui lòng chọn người dùng để xem thông tin")
                return

            username = values[0]
        
            # Tạo cửa sổ mới để hiển thị lịch sử mượn sách
            history_window = tk.Toplevel(self.main_frame)
            history_window.title(f"Lịch Sử Mượn Sách - {username}")
            history_window.geometry("800x600")  # Kích thước lớn hơn cho giao diện đẹp hơn
            history_window.configure(bg="#f5f5f5")  # Màu nền nhã nhặn

            # Tạo nhãn tiêu đề
            tk.Label(history_window, text=f"Lịch Sử Mượn Sách của {username}", font=("Helvetica", 16, "bold"), bg="#f5f5f5").pack(pady=10)

            # Bảng Treeview để hiển thị lịch sử mượn sách
            columns = ['STT', 'Tên Sách', 'Mã Sách', 'Ngày Mượn', 'Ngày Trả', 'Số Lượng', 'Trạng Thái']
            ...
            history_tree.column('STT', width=50, anchor='center')
            history_tree.column('Tên Sách', width=150)
            history_tree.column('Mã Sách', width=100, anchor='center')
            history_tree.column('Ngày Mượn', width=100, anchor='center')
            history_tree.column('Ngày Trả', width=100, anchor='center')
            history_tree.column('Số Lượng', width=80, anchor='center')
            history_tree.column('Trạng Thái', width=120, anchor='center')

            # Lấy dữ liệu lịch sử mượn sách từ cơ sở dữ liệu
            cur.execute("""
                SELECT b.book_name, b.isbn, b.borrow_date, b.return_date, b.quantity,
                CASE 
                    WHEN b.return_date < ? THEN 'Quá hạn'
                    ELSE 'Đã trả' 
                END AS status
                FROM books b
                WHERE b.borrowed_by = ?
            ...
            for idx, row in enumerate(cur.fetchall()):
                istory_tree.insert("", tk.END, values=(idx + 1, row[0], row[1], row[2], row[3], row[4], row[5]))

            # Đặt nút đóng cửa sổ
            tk.Button(history_window, text="Đóng", command=history_window.destroy, bg="#F5A9BC", fg="white").pack(pady=10)

        # Tạo các khung điều khiển và các nút chức năng
        frame_controls = tk.Frame(self.main_frame, bg='#d3d3d3', bd=10)
        frame_controls.pack(pady=10, padx=20, fill=tk.BOTH)
        ...
        tk.Button(frame_controls, text="Xem Thông Tin", width=17, command=view_borrow_history, bg="#F5A9BC", fg="white").grid(row=0, column=3, padx=10, pady=10)
        tk.Button(frame_controls, text="Quay Lại", width=17, command=self.show_admin_screen, bg="#F5A9BC", fg="white").grid(row=0, column=4, padx=10, pady=10)
        ...
        # Khung nhập thông tin người dùng
    ...
    def show_statistics(self):
        ...
        # Thêm nút in
        print_btn = ttk.Button(self.root, text="In Ra", command=lambda: self.print_statistics([["Số sách", 100], ["Số thành viên", 50]]))
